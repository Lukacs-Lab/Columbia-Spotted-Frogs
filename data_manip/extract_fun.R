...
# col_new is the name of the new column in the long dataset, made from all 
#   the wide columns
# sub_0 is replaces NAs with 0s
# If there are multiple columns that need to be combined into one column, then we 
#   do gather and add prim, sec columns
# If there is only one column to begin with, we just arrange it
extract_fun <- function(col_ext, col_new, sub_0 = T){
  #  Takes pattern to match using contains(), must be a character string that 
  #    matches some column(s) of the data
